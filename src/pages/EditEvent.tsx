...
const EditEvent: React.FC = () => {
  ...
  const mockEvent = {
    ...
    memo: "신랑과 신부 모두 친구입니다.",
    status: "upcoming"
  };
  
  // 폼 상태 (초기값은 기존 데이터로 설정)
  const [title, setTitle] = useState(mockEvent.title);
  const [eventType, setEventType] = useState(mockEvent.eventType);
  const [date, setDate] = useState(new Date(mockEvent.date));
  const [time, setTime] = useState(mockEvent.time);
  const [location, setLocation] = useState(mockEvent.location);
  const [memo, setMemo] = useState(mockEvent.memo);
